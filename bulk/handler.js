...
function orderFormater(orders) {
  const formatedOrders = _.map(orders, order => {
    ...
    // if (!_.isUndefined(tip) && !_.isObject(tip)) {
...
function eventFormater(events) {
  const formatedRes = _.map(events, event => {
